# -*- coding: utf-8 -*-
"""
Created on Sat May  9 21:39:06 2020
...
from sklearn.metrics import accuracy_score,hamming_loss,precision_score,multilabel_confusion_matrix,recall_score,f1_score,roc_curve,auc
...
classifiers=40#no of bootstrap samples to be trained
...
estimates=np.empty((80,2),dtype='float64')
for i in range(80):
...
fpr1,tpr1,thresholds1=roc_curve(y[:,0],estimates[:,0])
fpr2,tpr2,thresholds2=roc_curve(y[:,1],estimates[:,1])
fpr3,tpr3,thresholds3=roc_curve(y.ravel(),estimates.ravel())
plt.figure()
plt.plot(fpr1,tpr1)
plt.plot(fpr2,tpr2)
plt.plot(fpr3,tpr3)
plt.title('ROC curve for single hidden layered network')
plt.xlabel('false positive rate')
plt.ylabel('true positive rate')
plt.legend(['ROC curve of Hazards','ROC curve of Activity','micro average ROC curve'])
